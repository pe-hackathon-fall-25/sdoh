...
import { parseMaybeJson, coerceMessagesFromPayload, toTranscriptArray, mergeTranscripts } from '../services/transcript';
import { bufferTranscript } from '../services/voiceTranscription';
...
export function serializeCall(row: CallRow) {
  const transcript = toTranscriptArray(row.transcript);
  const detection =
    (row.detectionResult as DetectionResponse | null) || (row.analysis as DetectionResponse | null) || null;
...
r.post('/webhook/transcript', async (req, res, next) => {
  try {
    ...
    const existingTranscript = toTranscriptArray(call.transcript);
    const nextTranscript = messages.length ? mergeTranscripts(existingTranscript, messages) : existingTranscript;
